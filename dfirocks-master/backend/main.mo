...
shared(msg) actor class DFIRocks(
    _owner: Principal
    ) = this {
	...
	type TokenTxReceipt = Result.Result<Nat, {
        #InsufficientBalance;
    ...
    private stable var name_ : Text = "DRocks";
    private stable var symbol_ : Text = "DROCKS";
    private stable var owner_: Principal = _owner;
    private stable var totalSupply_: Nat = 0;
    private stable var blackhole: Principal = Principal.fromText("aaaaa-aa");
	private stable var xtc: TokenActor = actor("aanaa-xaaaa-aaaah-aaeiq-cai");
	private stable var mintPrice: Nat = 10_0000_0000; // 10 XTC for a rock
	private stable var maxSupply: Nat = 10000; // 10000 rocks
	...
	public shared(msg) func freeMint(to: Principal, url: Text, name: Text, desc: Text): async Bool {
		_mint(to, url, name, desc);
		return true;
	};

	public shared(msg) func mint(to: Principal, url: Text, name: Text, desc: Text): async Bool {
		switch (await xtc.transferFrom(msg.caller, owner_, mintPrice)) {
            case(#ok(id)) {};
    ...
    public shared(msg) func updateTokenInfo(info: TokenInfoExt) {
        assert(_exists(info.index));
		assert(_ownerOf(info.index) == ?msg.caller or msg.caller == owner_);
